...
public:
	...
	static bool editOneObj(const Car& obj);

	static bool addOneObj(std::string name, std::string type, double first, double sec, double mark);

	static bool deleteObjWhere(function<bool(const Car&)> comparer);

	static list<Car> findObjWhere(function<bool(const Car&)> comparer);
