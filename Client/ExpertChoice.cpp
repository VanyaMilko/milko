...
CExpertMethodDlg::CExpertMethodDlg(ClientConnectionModule* pModule, std::vector<Car>& lst, CWnd* pParent /*=nullptr*/)
...
void CExpertMethodDlg::AddToReport(Car& cloth)
{
	CString strFormat;
...
void CExpertMethodDlg::OnBnClickedNext()
{
	...
	if (m_wndReportCtrl.GetSelectedCount() != 5)
	{
		MessageBox(L"Нужно выбрать ровно 5 записей");
		return;
	}
